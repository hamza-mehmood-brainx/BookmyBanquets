...
const Dashboard = () => {
  ...
  const removeImage = async (index) => {
    ...
    try {
      await axios.post(
        '/api/v1/upload/hall-images/delete',
        { urls: [url] },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  useEffect(() => {
    const loadAmenities = async () => {
      try {
        const { data } = await axios.get("/api/v1/amenity")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const addrRes = await axios.post("/api/v1/address", addrPayload, {
        headers: { Authorization: `Bearer ${token}` }
      })
      ...
      const hallRes = await axios.post("/api/v1/hall", payload, {
        headers: { Authorization: `Bearer ${token}` }
      })
